...
class discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(128 * (self.input_height // 4) * (self.input_width // 4), 512),
            nn.BatchNorm1d(512),
            nn.LeakyReLU(0.2),
...
class LSGAN_gene(object):
    def __init__(self, args):
        ...
        # load dataset
        self.data_loader = utils.get_cat_loader(self.batch_size)
        self.z_loader = iter(utils.get_mat_loader(self.batch_size))
        print("dataset length:", len(self.data_loader))
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter_, x_ in enumerate(self.data_loader):
                if iter_ == self.data_loader.dataset.__len__() // self.batch_size:
                    break
                ...
                if ((iter_ + 1) % 100) == 0:
                    print("Epoch: [%2d] [%4d/%4d] D_loss: %.8f, G_loss: %.8f" %
                          ((epoch + 1), (iter_ + 1), self.data_loader.dataset.__len__() // self.batch_size, D_loss.data[0], G_loss.data[0]))
    ...
    def visualize_results(self, epoch, fix=False):
        ...
        if fix:
            ...
            """ random noise """
            try:
                sample_z_, _ = self.z_loader.next()
            except StopIteration:
                self.z_loader = iter(utils.get_mat_loader(self.batch_size))
                sample_z_, _ = self.z_loader.next()
            if self.gpu_mode:
