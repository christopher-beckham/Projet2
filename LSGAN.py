...
class generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512, 128 * (self.input_height // 4) * (self.input_width // 4)),
...
class discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(512 * (self.input_height // 16) * (self.input_width // 16), 1024),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, input):
        x = self.conv(input)
        x = x.view(-1, 512 * (self.input_height // 16) * (self.input_width // 16))
        x = self.fc(x)
        return x
'''
